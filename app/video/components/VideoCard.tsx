...
export function VideoCard({
  ...
  if (isPlaceholder || isLoading) {
    return (
      <div className="w-full aspect-video bg-[#F5F5F6] border border-[#E6E6EA] rounded-[24px] flex flex-col items-center justify-center gap-5 p-6">
        <div className="relative w-20 h-20 flex items-center justify-center">
          <svg className="absolute w-20 h-20 animate-spin" viewBox="0 0 80 80">
            {/* background circle */}
            <circle
              cx="40"
              cy="40"
        ...
        <div className="text-center z-10">
          <p className="text-[15px] font-bold text-[#110C0C] tracking-tight">
            Generate your video
          </p>
          <p className="text-[12px] text-[#8A8A8A] font-medium opacity-80 mt-1">
            Video will be ready in a few seconds
          </p>
        </div>
      </div>
    );
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[9999] bg-black/80 flex items-center justify-center p-4 md:p-12 animate-in fade-in duration-300 backdrop-blur-sm">
          ...
          <div className="relative max-w-5xl w-full bg-white rounded-[40px] p-8 overflow-hidden flex flex-col gap-6 animate-in zoom-in-95 duration-300 shadow-2xl">
            ...
            <div className="px-4">
              <p className="text-[16px] text-[#110C0C] font-bold leading-relaxed">
                {prompt}
              </p>
            </div>
          </div>

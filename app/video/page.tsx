...
export default function VideoPage() {
    ...
    async (showFullLoader = true) => {
      ...
      try {
        ...
        const transformed = data.map((item: any) => {
          // منطق التصنيف بناءً على الموديل
          const modelName = item.modelId?.name || "";
          const modelKey = item.modelId?.key || "";
          
          let itemType = "text-to-video"; // افتراضي
          
          if (modelName.includes("(I2V)") || modelKey.includes("image-to-video") || item.inputImageUrl) {
            itemType = "image-to-video";
          } else if (modelName.includes("(T2V)") || modelKey.includes("text-to-video")) {
            itemType = "text-to-video";
          } else {
            // fallback للحالات التي يكون فيها الموديل نل
            itemType = item.type === "image_to_video" ? "image-to-video" : "text-to-video";
          }
  ...
  const handleRegenerate = async (gen: any) => {
    setHistory((prev) =>
      prev.map((item) =>
        item.id === gen.id
          ? { ...item, isLoading: true, status: "processing" }
          : item,
      ),
    );
    try {
      ...
      setHistory((prev) =>
        prev.map((item) =>
          item.id === gen.id ? { ...item, isLoading: false } : item,
        ),
      );
    }
  };
  ...
  return (
    <PageLayout>
      ...
      <div className="flex flex-col lg:flex-row h-[calc(100vh-180px)] lg:h-[calc(100vh-90px)] bg-[#FEFBFB] overflow-hidden font-[Inter] rounded-[2px] border border-[#F0F0F3] shadow-sm">
        ...
        >
          <div className="px-6 py-4 flex items-center justify-between bg-[#F1F1F1] z-10">
            <div className="flex items-center gap-5">
              <div className="flex justify-center items-center px-4 lg:px-6 py-1 bg-[#F8F8F8] border border-[#E5E5E8] rounded-full">
                <div className="w-2 h-2 rounded-full bg-[#110C0C]/10" />

                <span className="px-2 text-[13px] font-bold text-[#110C0C] tracking-wider">
          ...
          <div className="flex-1 min-h-0 relative bg-[#F1F1F1]">
            {isLoading ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-[#8A8A8A] gap-6 lg:gap-8">
                <div className="w-20 h-20 lg:w-28 lg:h-28 bg-gray-50 rounded-[2.5rem] lg:rounded-[3rem] flex items-center justify-center border border-gray-100 shadow-inner">
                  <VideoIcon className="w-8 h-8 lg:w-12 lg:h-12 text-gray-300" />
                </div>
                <div className="text-center space-y-2 px-4">
                  <h3 className="text-[18px] lg:text-[20px] font-black text-[#110C0C] uppercase tracking-tighter">
                    Library Empty
                  </h3>
                  <p className="text-[13px] lg:text-[14px] font-medium max-w-[320px] leading-relaxed opacity-60">
                    Your {activeTab.replace("-", " ")} creations will appear
                    here.
                  </p>
                </div>
              </div>

  ...
  useEffect(() => {
    const fetchModels = async () => {
      ...
      try {
        ...
        const allScenes = modelsData.flatMap(m => (m.scenes || m.data?.scenes || []).map((s: any) => ({ 
          ...s, 
          isLocked: m.isLocked,
          modelName: m.name,
          provider: m.provider
        ...
        if (selectedScene && !allScenes.find(s => s.sceneId === selectedScene.sceneId)) {
    ...
    <div className="w-full lg:w-[440px] h-full bg-white border-r border-[#F0F0F3] flex flex-col font-[Inter] overflow-hidden">
      ...
      <div className="p-7 pb-4 border-b border-gray-50 bg-white/80 backdrop-blur-md z-20">
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            <h1 className="text-[18px] font-black text-[#110C0C] tracking-tight uppercase">AI Templates</h1>
            <p className="text-[11px] text-[#8A8A8A] font-bold uppercase tracking-widest">Kling AI Engine</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto elegant-scroll px-7 py-6 space-y-8">
        ...
        <section className="space-y-3">
          <div className="flex items-center justify-between px-1">
            <label className="text-[13px] font-black text-[#110C0C] uppercase tracking-tighter">Source Image</label>
            <span className="text-[10px] text-[#8A8A8A] font-bold">1:1 Recommended</span>
          </div>
          {inputImagePreview ? (
            <div className="relative group animate-in fade-in zoom-in-95 duration-300">
              <div className="w-full aspect-video rounded-[32px] overflow-hidden border-2 border-[#F0F0F3] bg-gray-50 shadow-sm relative">
              ...
              <button
                ...
                className="absolute -top-2 -right-2 w-9 h-9 bg-white text-[#110C0C] rounded-full flex items-center justify-center transition-all border border-[#E5E5E8] shadow-xl hover:scale-110 active:scale-95 z-10"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div
              ...
              className={cn(
                "w-full aspect-video rounded-[32px] border-2 border-dashed cursor-pointer transition-all duration-500 flex flex-col items-center justify-center gap-4 group bg-[#F9F9FB]",
                isDragging ? "border-red-500 bg-red-50/50" : "border-[#E5E5E8] hover:border-[#110C0C] hover:bg-white"
              )}
            >
              <div className="w-14 h-14 rounded-[22px] bg-white border border-[#E5E5E8] flex items-center justify-center text-[#8A8A8A] group-hover:scale-110 transition-transform shadow-sm">
                <ImageIcon className="w-7 h-7" />
              </div>
              <div className="text-center">
                <p className="text-[14px] font-black text-[#110C0C] uppercase tracking-tighter">Click or Drop Photo</p>
                <p className="text-[11px] text-[#8A8A8A] font-bold mt-1">MAX 10MB • JPG/PNG</p>
              </div>
        ...
        <section className="space-y-6">
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between px-1">
              <label className="text-[13px] font-black text-[#110C0C] uppercase tracking-tighter">Effect Filters</label>
              <div className="flex items-center gap-1.5 px-3 py-1 bg-gray-50 rounded-full border border-[#F0F0F3]">
                ...
                <span className="text-[10px] font-bold text-[#8A8A8A] uppercase">{filteredScenes.length} Found</span>
              </div>
            </div>
            ...
            <div className="flex items-center p-1 bg-[#F5F5F7] rounded-2xl w-full border border-[#E5E5E8] shadow-inner">
              <button 
                ...
                className={cn(
                  ...
                  faceOnly === null ? "bg-white text-[#110C0C] shadow-md border border-[#F0F0F3]" : "text-gray-400 hover:text-gray-600"
                )}
              >
                All
              </button>
              <button 
                ...
                className={cn(
                  "flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all",
                  faceOnly === true ? "bg-white text-[#110C0C] shadow-md border border-[#F0F0F3]" : "text-gray-400 hover:text-gray-600"
                )}
              >
              ...
              <button 
                ...
                className={cn(
                  "flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all",
                  faceOnly === false ? "bg-white text-[#110C0C] shadow-md border border-[#F0F0F3]" : "text-gray-400 hover:text-gray-600"
                )}
              >
                ...
                >
                  ...
                  {cat.sceneCount > 0 && (
                    <span className={cn(
                      ...
                      selectedCategory === cat.name ? "bg-white/20 text-white" : "bg-gray-100 text-[#8A8A8A]"
                    )}>
                      {cat.sceneCount}
                    </span>
                  )}
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between px-1">
            <label className="text-[13px] font-black text-[#110C0C] uppercase tracking-tighter">Select Style</label>
            <div className="w-5 h-5 rounded-full bg-blue-50 border border-blue-100 flex items-center justify-center cursor-help group/info relative">
               ...
               <div className="absolute bottom-full right-0 mb-2 w-48 p-3 bg-white border border-[#E5E5E8] rounded-xl shadow-xl text-[10px] font-medium text-gray-600 opacity-0 group-hover/info:opacity-100 transition-opacity z-50 pointer-events-none">
                 Styles marked with a person icon work best for selfies and portraits.
               </div>
            </div>
          </div>
          ...
          <div className="min-h-[260px]">
            {isLoading ? (
              <div className="grid grid-cols-2 gap-3">
                 {[1,2,3,4].map(i => (
                    <div key={i} className="h-32 bg-[#F9F9FB] rounded-[28px] border-2 border-transparent animate-pulse" />
                 ))}
              </div>
              ...
              <div className="py-16 text-center opacity-30 flex flex-col items-center gap-4 bg-[#F9F9FB] rounded-[32px] border-2 border-dashed border-[#E5E5E8]">
                ...
                <p className="text-[14px] font-black uppercase tracking-widest">No styles matched</p>
              </div>
            ) : (
                ...
                {filteredScenes.map(scene => (
                  <button
                    ...
                    className={cn(
                      ...
                      scene.isLocked && "opacity-40 grayscale cursor-not-allowed"
                    )}
                  >
                    <div className="flex items-center justify-between">
                      <div className={cn(
                        ...
                        selectedScene?.sceneId === scene.sceneId ? "bg-red-600 text-white" : "bg-[#F9F9FB] text-[#8A8A8A] border border-gray-100"
                      )}>
                        {scene.requiresFace ? <User className="w-6 h-6" /> : <Layers className="w-6 h-6" />}
                      </div>
                      {selectedScene?.sceneId === scene.sceneId && (
                    ...
                    <div>
                      <span className="text-[13px] font-black leading-tight block uppercase tracking-tighter truncate text-[#110C0C]">{scene.name}</span>
                      <div className="flex items-center gap-1.5 mt-1">
                        <img src="/assets/icons/ui/coin.svg" className="w-3 h-3" />
                        <span className="text-[10px] font-bold text-[#8A8A8A] uppercase tracking-widest">{scene.creditCost}</span>
                      </div>
                    </div>
                  </button>
                ))}
